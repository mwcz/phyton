...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Set image_hash to an md5 digest of each image's pixels."
        ...
        for photo in orm.Photo.objects.all():
            ...
            photo.image_hash = md5( img.tostring() ).hexdigest()
            photo.save()


    def backwards(self, orm):
        "Set the image_hash back to the (ridiculous) default."

        for photo in orm.Photo.objects.all():
            photo.image_hash = "01234567890123456789012345678901"
            photo.save()
